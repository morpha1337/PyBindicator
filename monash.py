...
def test_bin_data(bin_data: dict, wifi_controller) -> list:
    bins = [
...
def print_node_details(node: Element):
    print("TAG: ", node.tag)
...
def search_tree_by_tag(needle: str, stack: Element) -> list:
    arr = []
    if stack.tag == needle:
        arr.append(stack)
        return arr
...
def search_tree_by_attrib(attrib_name: str, attrib_value: str, stack: Element) -> list:
    arr = []
